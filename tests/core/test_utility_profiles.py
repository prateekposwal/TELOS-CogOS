...
class TestUtilityProfile:
    ...
    def test_aviku_profile_prioritizes_collaboration(self):
        # The real collaborative profile favors being helpful over being
        # strictly correct (per its own description + weights).
        assert AVIKU_PROFILE.weights[UtilityDimension.COLLABORATION] > \
            AVIKU_PROFILE.weights[UtilityDimension.CORRECTNESS]
...
class TestIdentityUtilityEngine:
    ...
    def test_selection_with_correctness_marker(self):
        from telos.core.identity.utility_profiles import AVIKU_PROFILE
        engine = IdentityUtilityEngine()
        profile = engine.select_profile(AVIKU_PROFILE.identity_markers)
