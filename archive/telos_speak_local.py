"""
TELOS Speaking — TELOS reasons, local LLM speaks (Ollama).
...
def telos_speak(user_input: str) -> str:
    ...
    if council_status == "BLOCKED":
        return f"[TELOS BLOCKED] Council rejected. DI={trace.decision_integrity:.3f}"

    reply = ollama_chat(
        f"You are TELOS, a Cognitive Operating System. Your reasoning trace: {reasoning}. Answer concisely.",
        user_input,
    )
    return reply

print("TELOS is speaking (local LLM). Type 'quit' to exit.\n")
while True:
    user = input("You: ")
