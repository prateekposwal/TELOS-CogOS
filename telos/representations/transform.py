...
class PolarTransform(RepresentationTransform):
    """Transforms Cartesian coordinates (x,y) to Polar (r, theta)."""
    ...
    def forward(self, input_data: Any) -> Any:
        # Assumes first two dims are x,y
        x, y = input_data[0], input_data[1]
        r = np.sqrt(x**2 + y**2)
        theta = np.arctan2(y, x)
        return np.array([r, theta], dtype=float)

    def inverse(self, output_data: Any) -> Any:
        r, theta = output_data[0], output_data[1]
        x = r * np.cos(theta)
        y = r * np.sin(theta)
