...
| 4.11 | **Cooperative Intelligence** | U_group − C_align ≥ max_i U_i | Cooperative aggregation is justified only when the crew's consensus utility — the authority-weighted mean over role lenses of the SAME primary decision — net of the alignment cost of its measured disagreement, is at least the best single agent acting alone. The crew is N role lenses over ONE decision, not separable agents, so per-lens scores must not be summed into a group total (that would double-count the decision); strict superadditivity (U_group > max_i U_i) is therefore not modelled, and the inequality holds exactly when measured divergence is zero | `coordination/cooperative.py` — `CooperativeCouncil` evaluates the crew's per-agent verdicts against the inequality (bounded, falsifiable scaffold; consensus mean, not a summable total) |
